...
for _ in range(trajs):
  ...
  if y[-1] > 2:
    ...
    if final_points is None:
      final_points = np.array([x[-1], y[-1]])
    else:
      final_points = np.vstack((final_points, np.array([x[-1], y[-1]])))
    no_collision += 1
...
ax.scatter(final_x_in, final_y_in, marker = '^', s=20, color='green')
ax.scatter(final_x_out, final_y_out, marker = '^', s=20, color='red')
