...
def run(multiagent=DEFAULT_MA, output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, colab=DEFAULT_COLAB, record_video=DEFAULT_RECORD_VIDEO, local=True, checkpoint=None, checkpoint_folder=None, filename=None, eval_set=None):
    ...
    if not multiagent:
        test_env = Monitor(EmpowermentAviary(gui=True,
                               obs=DEFAULT_OBS,
                               act=DEFAULT_ACT,
                               record=record_video,
                               eval_set=environment_set,
                               episode_len_sec=15))
        test_env_nogui = Monitor(EmpowermentAviary(obs=DEFAULT_OBS, act=DEFAULT_ACT, eval_set=environment_set, episode_len_sec=15))
    else:
    ...
    with open(csvfilename, 'a') as f:
        f.write('time,x,y,z,vx,vy,vz,mean_dist,avg_dist,lowest_dist,terminated\n')
    while True:
        ...
        if DEFAULT_OBS == ObservationType.KIN or DEFAULT_OBS == ObservationType.KINLID:
            if not multiagent:
                ...
                # average distance from obstacles, as the average of the lidar information
                mean_dist = np.mean(obs2[lidar_index:])
                # average
                avg_dist = np.average(obs2[lidar_index:])
                # lowest distance from obstacles, as the minimum of the lidar information
                lowest_dist = np.min(obs2[lidar_index:])
                with open(csvfilename, 'a') as f:
                    f.write(str(i/test_env.CTRL_FREQ) + ',' + str(obs2[0]) + ',' + str(obs2[1]) + ',' + str(obs2[2]) + ',' + str(obs2[10]) + ',' + str(obs2[11]) + ',' + str(obs2[12]) + ',' 
                            + str(mean_dist) + ',' + str(avg_dist) + ',' + str(lowest_dist) + ',' + str(terminated) + '\n')                
            else:
