...
class Maze:
    ...
    * v2 & v1: legacy versions in the [D4RL](https://github.com/Farama-Foundation/D4RL).
    ...
    def wall_locations(self) -> List[np.ndarray]:
        """Returns the x, y, z coordinates of the walls in the MuJoCo simulation"""
...
class MazeEnv(GoalEnv):
    ...
    def compute_collision(
        self, achieved_goal: np.ndarray, info, width =0.5
        ) -> bool:
        
        for wall in self.maze.wall_locations:
            #if euclidean between wall and achieved goal is greater than 4, don't compute collision
            if np.linalg.norm(wall - achieved_goal) > 4:
                continue
            wall_min = wall - width
            wall_max = wall + width            
            # wall_ij = self.maze.cell_xy_to_rowcol(wall)
            # wall_name = f"block_{wall_ij[0]}_{wall_ij[1]}"
            # clamp the sphere's center to the wall's bounds
            clamped = np.maximum(np.minimum(achieved_goal, wall_max), wall_min)
            # compute the distance between the sphere's center and the clamped position
            distance = np.linalg.norm(clamped - achieved_goal)
            # if the distance is less than the sphere's radius, a collision occurred            
            if distance < 0.2:      
                # change the color of the wall to red in mujoco to indicate collision                
                # tree = ET.parse(self.tmp_xml_file_path)
                # for geom in tree.findall(".//geom"):
                #     if geom.get("name") == wall_name:
                #         geom.set("rgba", "1 0 0 1")
                # tree.write(self.tmp_xml_file_path)
                return True
            # else:
                #paint the walls back to their original colors if no collision (0.7 0.5 0.3 1.0)
                # tree = ET.parse(self.tmp_xml_file_path)
                # for geom in tree.findall(".//geom"):
                #     if geom.get("name") == wall_name:
    ...
    def compute_empowerment(
        self, observation: np.ndarray, info
    ) -> float:
        final_points = None
        
        for _ in range(NUM_TRAJS):
           computed_trajectory = self.compute_trajectory(observation, info)
           if(self.compute_collision(computed_trajectory, info) == True):
               continue
           else:
               if final_points is None:
                   final_points = np.array([computed_trajectory[0], computed_trajectory[1]])
               else:
                   final_points = np.vstack((final_points, np.array([computed_trajectory[0], computed_trajectory[1]])))
    ...
    def compute_trajectory(
        ...
        F_x = F_MAX/4*np.polynomial.chebyshev.Chebyshev(coef=(2*np.random.uniform(size=N)-1)/(N/3), domain=[T_BEGIN, T_END], window=[-1,1])
        F_y = F_MAX/4*np.polynomial.chebyshev.Chebyshev(coef=(2*np.random.uniform(size=N)-1)/(N/3), domain=[T_BEGIN, T_END], window=[-1,1])
